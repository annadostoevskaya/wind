...
- JOIN RAK3172 разбит на бесконечные группы по три контролируемые попытки. Между
  группами RAK выключается, выполняются 5 секунд контроля вращения и проверка
  батареи; `PWR_OFF` разрешён только при `pulses == 0 AND BAT < 3,6 В`.
