...
- одна команда включает auto-join и максимум RUI3 — 255 внутренних попыток с
  интервалом 10 секунд; после их завершения STM32 снова выдаёт JOIN, поэтому
  общего лимита попыток нет;
- `AT_BUSY_ERROR` и duty-cycle restriction используют backoff и не увеличивают
  счётчик реальных radio failures;
- события неудачных попыток не останавливают внутренний цикл RAK; после каждых
  10 таких событий выполняется проверка батареи;
