...
| ACTIVE | Измерения, датчики и uplink | Потеря сети либо `BAT < 3,6 В AND pulses == 0` |
...
Raw-коды 0 и 4095 являются валидными результатами при `HAL_OK`. Код 0 означает
низкое напряжение, но сам по себе не разрешает отключение; код 4095 приводит к
saturation flag, но не к ложной ошибке ADC.

### 4.3 Вращение и условие отключения

Шесть импульсов соответствуют одному обороту. Основное отчётное окно близко к
RMS-окну (2333 мс). Для решения об отключении используется отдельное
10-секундное окно. Если оно завершилось без импульсов, прошивка проверяет
батарею и ещё раз считывает импульсы, накопленные во время этой проверки.

Отключение через `PWR_OFF` разрешено только логическим `AND` двух условий:

- достоверное напряжение батареи ниже 3,6 В;
- завершённое окно контроля вращения содержит 0 импульсов, и во время
  последующей проверки батареи не появился новый импульс.

Низкая батарея при продолжающемся вращении и нулевое вращение при нормальной
батарее не отключают устройство: измерения и uplink продолжаются.
...
- рабочий режим разрешает только точная полная строка `+EVT:JOINED`;
- `AT+NJS:1` не заменяет `+EVT:JOINED`: без финального события выполняется
  power-cycle и запускается новый наблюдаемый JOIN;
- первая команда останавливает сохранённый auto-join, после чего STM32 выдаёт
  по одной контролируемой попытке JOIN с интервалом 10 секунд;
- после трёх неудачных попыток RAK полностью обесточивается, `ON_FR` включается
  и выполняется отдельное 5-секундное окно вращения, затем измеряется батарея;
- если за эти 5 секунд получено 0 импульсов и батарея ниже 3,6 В, выполняется
  полное отключение через `PWR_OFF`; если хотя бы одно условие не выполнено или
  батарея не прочитана достоверно, RAK включается и получает следующие три
  попытки; поэтому общий лимит JOIN отсутствует;
- `AT_BUSY_ERROR` и duty-cycle restriction используют backoff и не увеличивают
  счётчик реальных radio failures;
- перед первой командой RAK получает 5 секунд на загрузку; отсутствие ответа
  на JOIN или последующий `AT+NJS` приводит к отключению питания на 1 секунду,
  повторному включению и новому бесконечному циклу JOIN;
- ответы классифицируются как полные строки, без substring false positives.

### 5.2 Uplink

Первый пакет формируется сразу после первого валидного окна RMS, готовности
батареи и завершения текущего цикла DS18B20. Последующие пакеты запрашиваются
каждые 5 000 мс. Значение можно переопределить через `UPLINK_PERIOD_MS`.
Фактический темп в эфире ограничивается регионом, data rate, airtime и duty
cycle самого RAK3172: при `AT_BUSY_ERROR` или duty-cycle restriction прошивка
повторяет запрос через 5 секунд.
...
Если команда не ушла в UART, RAK не ответил в установленный таймаут, вернул
ошибку SEND или сообщил потерю сети, STM32 выключает питание RAK на 1 секунду,
включает его и снова ожидает `+EVT:JOINED`.
